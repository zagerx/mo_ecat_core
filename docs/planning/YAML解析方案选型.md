...
```
config/master.yaml                  # 主站 + 总线 + 后端配置
applications/robot/config/robot.yaml # 机器人应用配置
...
```bash
mo_ecat_config_gen \
  --master-yaml config/master.yaml \
  --robot-yaml applications/robot/config/robot.yaml \
...
```bash
mo_ecat_master_config_gen  config/master.yaml  build/generated/mo_ecat_master_cfg
mo_ecat_robot_config_gen   applications/robot/config/robot.yaml  build/generated/robot_cfg
```
...
add_custom_command(
    ...
    COMMAND mo_ecat_config_gen
        --master-yaml ${CMAKE_SOURCE_DIR}/config/master.yaml
        --robot-yaml ${CMAKE_SOURCE_DIR}/applications/robot/config/robot.yaml
    ...
    DEPENDS
        mo_ecat_config_gen
        ${CMAKE_SOURCE_DIR}/config/master.yaml
        ${CMAKE_SOURCE_DIR}/applications/robot/config/robot.yaml
...
- 保留 `robot_default_config()` 和 `mo_ecat_master_options` 的默认初始化逻辑。
...
- 生成的 `.c/.h` 顶部添加版本注释，方便追踪：
  ```c
  /* Auto-generated by mo_ecat_config_gen v1.0 from config/master.yaml */
...
| 输入 | `config/master.yaml` + `applications/robot/config/robot.yaml` |
...
6. 改造 CLI，用生成的 `g_master_options` 和 `g_user_config` 替代当前硬编码。
