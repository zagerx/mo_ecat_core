 ...
 * @bit_length: 周期数据字段位宽，例如 0x60400420 表示 32 bit
 * @direction: 主站视角的数据方向；RxPDO 0x1600 中的字段为 OUTPUT，
 *             TxPDO 0x1A00 中的字段为 INPUT
 *
 * 例如，slave_index=2 的从站在 0x1600:03 中映射了
 * 0x6040:04/32 bit，Master 可能将其公开为：
 *
 * entry_id=17、slave_index=2、object_index=0x6040、
 * object_subindex=0x04、bit_length=32、
 * direction=MO_ECAT_PDO_OUTPUT。
 *
 * pdo_index=0x1600 和 pdo_subindex=3 属于 Core 私有扫描上下文，
 * 当前不通过该公共结构暴露；字段在周期数据区域内的物理偏移也由
 * Core 保存，应用层不直接访问。
 */
