...
ServoNode::ServoNode(EcMaster &master, const SlaveConfig &config)
...
const SlaveConfig &ServoNode::GetConfig() const
{
    return config_;
}

ServoAxis &ServoNode::GetAxis()
{
    return axis_;
}

const PdoEntry *ServoNode::FindEntry(const std::vector<PdoEntry> &entries, const std::string &name)
{
...
size_t ServoNode::ComputeBufferSize(const std::vector<PdoEntry> &entries)
{
    size_t size = 0;
    for (const auto &entry : entries) {
...
bool ServoNode::RequestState(uint16_t state)
{
    if (!master_.RequestState(config_.slave_id, state)) {
        LOG_WARN << "ServoNode " << config_.name << " failed to request state " << state;
        return false;
    }
...
uint16_t ServoNode::GetCurrentState() const
{
    return master_.GetCurrentState(config_.slave_id);
}

bool ServoNode::SdoRead(uint16_t index, uint8_t subindex, void *data, size_t len, int timeout_us)
{
    return master_.SdoRead(static_cast<uint16_t>(config_.slave_id), index, subindex, data,
                           static_cast<int>(len), timeout_us);
}

bool ServoNode::SdoWrite(uint16_t index, uint8_t subindex, const void *data, size_t len, int timeout_us)
{
    return master_.SdoWrite(static_cast<uint16_t>(config_.slave_id), index, subindex, data,
                            static_cast<int>(len), timeout_us);
}

bool ServoNode::ConfigurePdoMapping()
{
    // 框架阶段，后续通过 SDO 配置 PDO 映射
    LOG_INFO << "ServoNode " << config_.name << " ConfigurePdoMapping() called (framework only)";
    return true;
}

void ServoNode::SetPdoMapping(const PdoMapping &mapping)
{
...
void ServoNode::UpdatePdoOutput()
{
...
void ServoNode::UpdatePdoInput()
{
    ...
    double position = static_cast<double>(position_raw) * axis_cfg.position_scale * axis_cfg.direction;
    double velocity = static_cast<double>(velocity_raw) * axis_cfg.velocity_scale * axis_cfg.direction;
    double torque = static_cast<double>(torque_raw) * axis_cfg.torque_scale * axis_cfg.direction;
