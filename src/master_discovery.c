...
static void *aligned_region(void *memory, size_t *offset,
...
static int allocate_discovery_memory(struct mo_ecat_master *master,
                                     size_t slave_count)
{
    size_t slave_size;
    size_t state_size;
    ...
    if (add_aligned_size(&total_size, _Alignof(struct mo_ecat_slave),
                         slave_size) < 0 ||
        add_aligned_size(&total_size, _Alignof(struct mo_ecat_slave_state),
                         state_size) < 0) {
        return -1;
    }
    ...
    master->diag.slaves = aligned_region(master->runtime_memory.memory,
                                         &offset,
                                         _Alignof(struct mo_ecat_slave),
                                         slave_size);
    master->diag.states = aligned_region(master->runtime_memory.memory,
                                         &offset,
                                         _Alignof(struct mo_ecat_slave_state),
                                         state_size);
    return 0;
}
...
int master_prepare_discovery(struct mo_ecat_master *master)
{
    ...
    if (!master || !master->backend.ops || !master->backend.ops->scan) {
        return -1;
    }

    if (master->backend.ops->scan(&master->backend, &slave_count) < 0 ||
        allocate_discovery_memory(master, slave_count) < 0) {
        goto fail;
    }
    ...
    if (slave_count > 0 &&
        (!master->backend.ops->read_discovered_slaves ||
         master->backend.ops->read_discovered_slaves(&master->backend,
                                                     master->diag.slaves,
                                                     slave_count) < 0)) {
        goto fail;
    }
    if (master->backend.ops->read_diagnostics) {
        if (master->backend.ops->read_diagnostics(&master->backend,
                                                  master->diag.states,
                                                  master->diag.count) < 0) {
            goto fail;
        }
